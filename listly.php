	...
	class Listly
	{
		...
		function ListlyAJAXPublisherAuth()
		{
			...
			if ( isset( $_POST['action'], $_POST['Key'] ) )
			{
				...
				{
					add_action( 'http_api_debug', array( $this, 'DebugHTTP' ), 10, 5 );
					...
					$HTTPErrorLog = get_transient( 'Listly-HTTP-ErrorLog' );

					if ( is_wp_error( $Response ) )
					{
						printf( '<span class="error">%s<br>Error: %s <br>Debug Info:<br><textarea class="large-text" rows="4">%s</textarea></span>', $ErrorMessage, $Response->get_error_message(), $HTTPErrorLog );

						wp_mail( 'support@list.ly', 'Listly Plugin HTTP Error ' . site_url(), sprintf( "WP URL: %s\n\nPublisher Key: %s\n\nDebug Info: %s", site_url(), sanitize_text_field( $_POST['Key'] ), $HTTPErrorLog ) );
					}
					elseif ( 200 != $Response['response']['code'] )
					{
						printf( '<span class="error">%s<br>Error: %s - %s <br>Debug Info:<br><textarea class="large-text" rows="4">%s</textarea></span>', $ErrorMessage, $Response['response']['code'], $Response['response']['message'], $HTTPErrorLog );

						wp_mail( 'support@list.ly', 'Listly Plugin HTTP Error ' . site_url(), sprintf( "WP URL: %s\n\nPublisher Key: %s\n\nDebug Info: %s", site_url(), sanitize_text_field( $_POST['Key'] ), $HTTPErrorLog ) );
					}
					elseif ( $Response['body'] == '' )
					{
						printf( '<span class="error">%s<br>Error: No data returned <br>Debug Info:<br><textarea class="large-text" rows="4">%s</textarea></span>', $ErrorMessage, $HTTPErrorLog );
					...
					remove_action( 'http_api_debug', array( $this, 'DebugHTTP' ), 10 );
				}
			}
		...
		function ShortCode( $Attributes, $Content = null, $Code = '' )
		{
			...
			if ( false === ( $Response = get_transient( $TransientId ) ) )
			{
				...
				if ( ! is_wp_error( $Response ) && isset( $Response['body'] ) && $Response['body'] != '' )
				{
					...
					if ( $ResponseJson['status'] == 'ok' )
					{
						if ( set_transient( $TransientId, $Response, 86400 ) == false )
						{
							$this->DebugConsole( 'Cache creation failed', false, $ListId );
						}
					}
				}
			}
				...
				if ( false !== ( $Timeout = get_option( "_transient_timeout_$TransientId" ) ) && $Timeout < time() + 82800 )
				{
					...
					if ( ! is_wp_error( $ResponseFresh ) && isset( $ResponseFresh['body'] ) && $ResponseFresh['body'] != '' )
					{
						...
						if ( $ResponseFreshJson['status'] == 'ok' )
						{
							...
							if ( set_transient( $TransientId, $ResponseFresh, 86400 ) == false )
							{
								$this->DebugConsole( 'Cache update failed', false, $ListId );
							}
		...
		function DebugHTTP( $response, $context, $class, $args, $url )
		{
			set_transient( 'Listly-HTTP-ErrorLog', 'Request Arguments: ' . print_r( $args, true ) . "\n\n" . 'Response : ' . print_r( $response, true ), 60 );
		}
